...
router.put('/mk/diseases/:id', async (req, res, next) => {
  const { name, icd10, category, age_group, sex, description, reasoning, red_flags } = req.body;
  const { rows } = await db.query(
...
router.delete('/mk/diseases/:id', async (req, res, next) => {
...
/* ── Keywords ── */
router.get('/mk/diseases/:id/keywords', async (req, res, next) => {
  const { rows } = await db.query(
    `SELECT * FROM disease_keywords WHERE disease_id=$1 ORDER BY id`, [req.params.id]);
  res.json({ ok: true, keywords: rows });
});
router.post('/mk/diseases/:id/keywords', async (req, res, next) => {
...
router.delete('/mk/keywords/:id', async (req, res, next) => {
...
/* ── Investigations ── */
router.get('/mk/diseases/:id/investigations', async (req, res, next) => {
  const { rows } = await db.query(
...
router.post('/mk/diseases/:id/investigations', async (req, res, next) => {
...
router.delete('/mk/treatments/:id', async (req, res, next) => {
...
router.put('/mk/symptoms/:id', async (req, res, next) => {
  const { name, body_system, description } = req.body;
  const { rows } = await db.query(
...
router.delete('/mk/symptoms/:id', async (req, res, next) => {
...
router.put('/mk/questions/:id', async (req, res, next) => {
  const { section, question, importance, follow_up_for, triggers } = req.body;
  const { rows } = await db.query(
...
router.delete('/mk/questions/:id', async (req, res, next) => {
...
════════════════════════════════════════════════════════════ */
router.post('/sql/upload', upload.single('file'), async (req, res, next) => {
...
════════════════════════════════════════════════════════════ */
router.get('/mk/synonyms', async (_req, res, next) => {
  try {
...
router.post('/mk/synonyms', async (req, res, next) => {
...
router.put('/mk/synonyms/:id', async (req, res, next) => {
  const { canonical, synonym } = req.body;
  if (!canonical?.trim()) return res.status(400).json({ ok: false, error: 'canonical is required' });
  if (!synonym?.trim())   return res.status(400).json({ ok: false, error: 'synonym is required' });
  try {
...
router.delete('/mk/synonyms/:id', async (req, res, next) => {
  try {
...
router.get('/facility', async (_req, res, next) => {
  try {
...
router.put('/facility', async (req, res, next) => {
  const {
...
router.get('/sha-schedule/packages', async (_req, res, next) => {
  try {
    const { rows } = await db.query(
...
router.get('/sha-schedule/items', async (req, res, next) => {
...
router.get('/sha-schedule/surgical', async (req, res, next) => {
 ...
 */
/**
 * POST /api/admin/epidemiology/backfill-chosen
 * One-time fix: for all closed encounters where diagnoses.chosen is empty
 * but diagnoses.provisional.name exists, copies provisional into chosen.
 */
router.post('/epidemiology/backfill-chosen', async (req, res, next) => {
  try {
    const { rows } = await db.query(`
 ...
 */
router.get('/epidemiology/top-diagnoses', async (req, res, next) => {
  try {
    ...
    const { rows } = await db.query(`
      WITH dx_flat AS (
        SELECT
          ...
          COALESCE(
            (dx->>'confidence')::numeric,
            (dx->>'percentage')::numeric
          )                                  AS confidence
        FROM encounters e
      ...
      provisional_flat AS (
        SELECT
          ...
          d.icd10,
          COALESCE(
            (e.diagnoses->'provisional'->>'confidence')::numeric,
            (e.diagnoses->'provisional'->>'percentage')::numeric
          ) AS confidence
        FROM encounters e
        JOIN patients p ON p.id = e.patient_id
 ...
 */
router.get('/epidemiology/trends', async (req, res, next) => {
  try {
 ...
 */
router.get('/epidemiology/category-breakdown', async (req, res, next) => {
  try {
    const days = parseInt(req.query.days || '30', 10);
 ...
 */
router.get('/epidemiology/alerts', async (req, res, next) => {
  try {
 ...
 */
router.get('/epidemiology/by-doctor', async (req, res, next) => {
  try {
    const days = parseInt(req.query.days || '30', 10);
    const { rows } = await db.query(`
      SELECT
